...
<div class="container" style="margin-top: 20px;">
    ...
    <div class="bottom-buffer top-buffer">
        ...
        <ul>
            ...
            <li>Weicht die Antwort leicht ab (Zustimmung/Neutral oder Neutral/Ablehnung), wird den GRÜNEN 1 Punkt
                gutgeschrieben;
            </li>
            <li>Sind die Antworten entgegengesetzt, gibt es keine Punkte für die Gruppe.</li>
        </ul>
        <p>Eine Frage, die die Testperson übersprungen hat, wird nicht gewertet. Entsprechend gibt es dann insgesamt
            weniger Punkte zu erreichen.</p>
        <p>Eine Frage, die doppelt gewichtet werden soll, wird doppelt gewichtet, das heißt, für sie wird die doppelte
            Punktzahl gutgeschrieben (0/2/4). Entsprechend gibt es insgesamt mehr Punkte zu erreichen.</p>

        <div id='log'>
            <h4>Werden meine Antworten gespeichert?</h4>
            <p>Vor der Auswertung wirst du gefragt, ob wir deine Antworten speichern dürfen.<br>
                Du kannst dann "Ja" auswählen, was bedeutet, dass wir deine Antwortkombination zusammen mit einer ID
                speichern, die aus deiner IP-Adresse und einem täglich wechselnden zufälligen Wert errechnet wird.
                Bereits am nächsten Tag kann dein Eintrag auf keinen Fall mehr einer konkreten IP-Adresse zugeordnet
                werden.<br>
                Wenn du "Nein" auswählst, wird lediglich ein Zähler um 1 erhöht.</p>
            <p></p>

            <h4>Ich möchte gar nicht gezählt werden!</h4>
            <p>Unser Webserver legt bei jedem Seitenaufruf einen Log-Eintrag an, der unter anderem einen Zeitstempel,
                deine IP-Adresse und die aufgerufene URL enthält. Es wäre also unaufrichtig, dir vorzumachen, dass dein
                Aufruf des Mahlowat nicht gezählt wird. Der Mahlowat wurde jedoch so konzipiert, dass aus den
                Server-Logdateien nicht ersichtlich ist, welche Antworten du ausgewählt hast. Dies sehen wir tatsächlich
                nur, wenn du dem am Ende explizit zustimmst.</p>
        </div>

        <h4>Ich habe einen Fehler gefunden!</h4>
        <p>Dann solltest du das melden. Wir freuen uns über sachdienliche Hinweise.</p>

        <a class="btn btn-primary" href="<?php echo $back; ?>" onclick="callPage(event, '<?php echo $back; ?>')"
           title="Zurück zum Mahlowat">Zurück zum Green-o-mat</a>
    </div>
</div>

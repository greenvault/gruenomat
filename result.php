...
<div class="container" style="margin-top: 20px;">
    ...
    <div class="bottom-buffer top-buffer">
        ...
        <?php if ($warning && !isset($_GET['ans'])) { ?>
            <div id="warning" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel"
                 aria-hidden="true">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <p><strong>Anscheinend hast du keine Fragen beantwortet.</strong><br/>
                                Entweder hast du diese Seite direkt aufgerufen, oder du hast die Thesen wirklich noch
                                nicht bearbeitet.</p>
        ...
        <?php if (!$bars_only) { ?>
            <p>
                <small>Nicht zufrieden mit dem Ergebnis? Vielleicht willst du die Thesen <a href="multiplier.php"
                                                                                            onclick="callPage(event, 'multiplier.php', <?php echo "'$answerstring', '$count'"; ?>)"
                                                                                            title="Gewichtung ändern">anders
                        gewichten</a>.
                </small>
            </p>
        <?php } ?>
        ...
        <div id="result-bars">
            <table class="table table-bordered table-hover">
                <tr>
                    <th style="width: 200px;">Liste</th>
                    <th style="width:100px">Punkte</th>
                    <th style="width:640px;">Punkte</th>
                </tr>
                <?php
        ...
        <?php if (!$bars_only) { ?>
            <div id="result-table">
                ...
                <p>
                    <small>Thesen mit <span class="glyphicon glyphicon-star" title="Sternchen"></span> fandest du
                        besonders wichtig.<br> Wenn du auf den Button mit dem Namen der These klickst, bekommst du die
                        Statements der GRÜNEN in einer Übersicht angezeigt.
                    </small>
                </p>
        ...
        <div class="control-group alert alert-info">
            ...
            <div class="controls sharecontrols">
                <input type="text" class="col-md-5 form-control" id="resultlink" placeholder=""
                       value="<?php echo $sharelink; ?>">
            </div>
            <p><?php if ($count === 'false') { ?>
                    <strong>Achtung!</strong> Aus diesem Link kann man ablesen, welche Antworten du ausgewählt und wie du die Thesen gewichtet hast!<?php } ?>
                &nbsp;</p>
        </div>
        ...
        <div class="text-right">
            <small>Du kannst die Befragung
                <a href="index.php" title="Von vorn beginnen">neu starten</a><?php if ($bars_only) {
                    echo '.';
                } else { ?>,
                    deine
                    <a href="mahlowat.php"
                       onclick="callPage(event, 'mahlowat.php', <?php echo "'$answerstring', '$count'"; ?>)"
                       title="Antworten ändern">Antworten ändern</a>
                    oder die
                    <a href="multiplier.php"
                       onclick="callPage(event, 'multiplier.php', <?php echo "'$answerstring', '$count'"; ?>)"
                       title="Gewichtung ändern">Gewichtung anpassen</a>.<?php } ?><br/>
                Außerdem haben wir auch eine <a href="faq.php?from=result.php<?php if ($share_via_id) {
                    echo $sharelink;
                } ?>" onclick="callPage(event, 'faq.php?from=result.php<?php if ($share_via_id) {
                    echo $sharelink;
                }
                echo "', '$answerstring', '$count'"; ?>)" title="FAQ">FAQ-Seite</a>.
            </small>
        </div>
    </div>
</div>
